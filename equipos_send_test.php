...
$peopleById = [];
foreach ($people as $p) {
    $user = trim((string)($p['telegram_user'] ?? ''));
    if ($user !== '' && strpos($user, '@') !== 0) {
        $user = '@' . $user;
    }
    $peopleById[(int)$p['id']] = $user !== '' ? $user : (string)$p['name'];
}
    ...
    foreach ($orderedObjectives as $objective) {
        ...
        if ($independent) {
            ...
            foreach ($independent as $task) {
                ...
                $lines[] = '- ' . $responsiblesText . ' tiene que ' . $taskName . ' antes del ' . $taskDue . '.';
                $mentionedIds[(int)$task['id']] = true;
            }
        }
        foreach ($chainRoots as $rootTask) {
            $mentionedIds[(int)$rootTask['id']] = true;
            $dependents = $children[(int)$rootTask['id']] ?? [];
            $dependentTask = null;
            if ($dependents) {
                foreach ($open as $t) {
            ...
            if ($dependentTask) {
                $mentionedIds[(int)$dependentTask['id']] = true;
            }
        }
