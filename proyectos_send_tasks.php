...
if (!$targets) {
    $_SESSION['flash_error'] = 'No hay grupos configurados en los equipos del proyecto.';
    header('Location: /proyectos_list.php');
    exit;
}
...
foreach ($targets as $target) {
    ...
    if (!$result['ok']) {
        ...
        $failed[] = $target['name'] . ' (' . $code . ($detail !== '' ? ': ' . $detail : '') . ')';
    }
}
...
if ($failed) {
    $_SESSION['flash_error'] = 'Falló el envío a: ' . implode(' | ', $failed);
} else {
    $_SESSION['flash_success'] = 'Mensaje enviado a los equipos del proyecto.';
}

if ($userFailures) {
    $prefix = $failed ? ($_SESSION['flash_error'] . ' ') : '';
    $_SESSION['flash_error'] = trim($prefix . 'Checklists no enviadas: ' . implode(' | ', $userFailures));
}
