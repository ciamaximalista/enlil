...
$peopleIds = $_GET['people'] ?? [];
if (!is_array($peopleIds)) {
    $peopleIds = [$peopleIds];
}
$peopleIds = array_values(array_filter(array_map('intval', $peopleIds)));
if (count($peopleIds) < 2) {
    header('Location: /calendarios_list.php?compare_error=1');
    exit;
}

$people = [];
foreach ($peopleIds as $pid) {
    $person = enlil_people_get($pid);
    if ($person) {
        $people[] = $person;
    }
}
    ...
    foreach ($project['objectives'] as $objective) {
        foreach ($objective['tasks'] as $task) {
            ...
            foreach ($people as $person) {
                if (in_array((int)$person['id'], $task['responsible_ids'], true)) {
                    $tasks[] = [
                        'date' => $task['due_date'],
                        'name' => $task['name'],
...
function enlil_month_matrix_compare(int $baseDate): array {
...
function enlil_week_days_compare(int $baseDate): array {
...
function enlil_format_month_es_compare(int $ts): string {
    $months = [
...
$peopleNames = array_map(function ($p) {
    return $p['name'];
}, $people);
$peopleParam = '';
foreach ($people as $p) {
    $peopleParam .= '&people[]=' . (int)$p['id'];
}

enlil_page_header('Comparar calendarios');
?>
    <main class="container">
        <div class="page-header">
            <div>
                <h1>Comparar calendarios</h1>
                <p class="muted"><?php echo htmlspecialchars(implode(', ', $peopleNames)); ?></p>
            </div>
            <div class="actions">
                <a class="btn secondary" href="/calendarios_list.php">Volver</a>
                <a class="btn small <?php echo $view === 'week' ? 'secondary' : ''; ?>" href="?view=month&date=<?php echo date('Y-m-d', $baseDate); ?><?php echo $peopleParam; ?>">Mensual</a>
                <a class="btn small <?php echo $view === 'month' ? 'secondary' : ''; ?>" href="?view=week&date=<?php echo date('Y-m-d', $baseDate); ?><?php echo $peopleParam; ?>">Semanal</a>
            </div>
        </div>

        <div class="section-card calendar-card">
            <div class="calendar-header">
                <strong><?php echo htmlspecialchars(enlil_format_month_es_compare($baseDate)); ?></strong>
                <div class="actions">
                    <?php if ($view === 'month'): ?>
                        <?php if ($canPrevMonth): ?>
                            <a class="btn small secondary" href="?view=month&date=<?php echo date('Y-m-d', strtotime('-1 month', $baseDate)); ?><?php echo $peopleParam; ?>">‹</a>
                        <?php endif; ?>
                        <?php if ($canNextMonth): ?>
                            <a class="btn small secondary" href="?view=month&date=<?php echo date('Y-m-d', strtotime('+1 month', $baseDate)); ?><?php echo $peopleParam; ?>">›</a>
                        <?php endif; ?>
                    <?php else: ?>
                        <?php if ($canPrevWeek): ?>
                            <a class="btn small secondary" href="?view=week&date=<?php echo date('Y-m-d', strtotime('-7 days', $baseDate)); ?><?php echo $peopleParam; ?>">‹</a>
                        <?php endif; ?>
                        <?php if ($canNextWeek): ?>
                            <a class="btn small secondary" href="?view=week&date=<?php echo date('Y-m-d', strtotime('+7 days', $baseDate)); ?><?php echo $peopleParam; ?>">›</a>
                        <?php endif; ?>
            ...
            <?php if ($view === 'month'): ?>
                <div class="calendar-grid">
                    ...
                    <?php foreach (enlil_month_matrix_compare($baseDate) as $row): ?>
                                    ...
                                    <?php if (!empty($tasksByDate[$dayKey])): ?>
                                        <?php foreach ($tasksByDate[$dayKey] as $task): ?>
                                            <div class="calendar-task">
                                                <span class="accent red"><?php echo htmlspecialchars($task['person']); ?></span> · <span class="accent"><?php echo htmlspecialchars($task['project']); ?></span> · <?php echo htmlspecialchars($task['objective']); ?> · <?php echo htmlspecialchars($task['name']); ?>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
            ...
            <?php else: ?>
                <div class="calendar-grid week">
                    <?php foreach (enlil_week_days_compare($baseDate) as $dayTs): ?>
                        <?php $dayKey = date('Y-m-d', $dayTs); ?>
