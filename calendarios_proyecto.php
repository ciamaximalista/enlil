    ...
    foreach ($objective['tasks'] as $task) {
        ...
        foreach ($task['responsible_ids'] as $personId) {
            if (isset($peopleById[$personId])) {
                $responsibleNames[] = $peopleById[$personId];
            }
        }
        $tasks[] = [
            'date' => $task['due_date'],
...
enlil_page_header('Calendario de proyecto');
?>
    <main class="container">
        <div class="page-header">
            <div>
                <h1><?php echo htmlspecialchars($project['name']); ?></h1>
                <p class="muted">Calendario del proyecto</p>
            </div>
            <div class="actions">
                <a class="btn secondary" href="/calendarios_list.php">Volver</a>
                <a class="btn small <?php echo $view === 'week' ? 'secondary' : ''; ?>" href="?id=<?php echo (int)$projectId; ?>&view=month&date=<?php echo date('Y-m-d', $baseDate); ?>">Mensual</a>
                <a class="btn small <?php echo $view === 'month' ? 'secondary' : ''; ?>" href="?id=<?php echo (int)$projectId; ?>&view=week&date=<?php echo date('Y-m-d', $baseDate); ?>">Semanal</a>
            </div>
        </div>
        ...
        <div class="section-card calendar-card">
            <div class="calendar-header">
                ...
                <div class="actions">
                    <?php if ($view === 'month'): ?>
                        <?php if ($canPrevMonth): ?>
                            <a class="btn small secondary" href="?id=<?php echo (int)$projectId; ?>&view=month&date=<?php echo date('Y-m-d', strtotime('-1 month', $baseDate)); ?>">‹</a>
                        <?php endif; ?>
                        <?php if ($canNextMonth): ?>
                            <a class="btn small secondary" href="?id=<?php echo (int)$projectId; ?>&view=month&date=<?php echo date('Y-m-d', strtotime('+1 month', $baseDate)); ?>">›</a>
                        <?php endif; ?>
                    <?php else: ?>
                        <?php if ($canPrevWeek): ?>
                            <a class="btn small secondary" href="?id=<?php echo (int)$projectId; ?>&view=week&date=<?php echo date('Y-m-d', strtotime('-7 days', $baseDate)); ?>">‹</a>
                        <?php endif; ?>
                        <?php if ($canNextWeek): ?>
                            <a class="btn small secondary" href="?id=<?php echo (int)$projectId; ?>&view=week&date=<?php echo date('Y-m-d', strtotime('+7 days', $baseDate)); ?>">›</a>
                        <?php endif; ?>
                                    ...
                                    <?php if (!empty($tasksByDate[$dayKey])): ?>
                                        <?php foreach ($tasksByDate[$dayKey] as $task): ?>
                                            <div class="calendar-task">
                                                ...
                                                <?php if (!empty($task['responsibles'])): ?>
                                                    <span class="muted">· <span class="accent"><?php echo htmlspecialchars(implode(', ', $task['responsibles'])); ?></span></span>
                                                <?php endif; ?>
                                            </div>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
